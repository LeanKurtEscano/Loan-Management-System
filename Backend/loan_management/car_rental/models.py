...
class CarLoanApplication(models.Model):
    ...
    car_id = models.IntegerField(blank=True, null=True)  # Unique identifier for the car
    first_name = models.CharField(max_length=100)
    middle_name = models.CharField(max_length=100, blank=True, null=True)
    last_name = models.CharField(max_length=100)
    birthdate = models.DateField()
    front_id = CloudinaryField('front_image', blank=True, null=True)  # Cloudinary Field
    back_id = CloudinaryField('back_image', blank=True, null=True)
    ...
    interest = models.DecimalField(max_digits=10, decimal_places=2, default=0.00, blank=True, null=True)  
...
class CarLoanCommission(models.Model):
    
    payment = models.ForeignKey(CarLoanPayments, on_delete=models.CASCADE)
    amount = models.DecimalField(max_digits=10, decimal_places=2)
    track_system_amount = models.DecimalField(max_digits=10, decimal_places=2, default=0.00)
    created_at = models.DateTimeField(auto_now_add=True, blank=True, null=True)