...
const ManageRentals: React.FC = () => {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries(['carloanApplicationsAdmin']);
    }
  });
  ...
  const pendingApplications = loanApplications.filter(app => app.status === "Ongoing").length;
  const approvedApplications = loanApplications.filter(app => app.status === "Completed").length;

  const stats: StatCard[] = [
    { title: "Total Disbursements", value: totalApplications, icon: faClipboardList, color: "from-blue-400 to-blue-600" },
    { title: "Ongoing Disbursements", value: pendingApplications, icon: faClock, color: "from-yellow-400 to-yellow-600" },
    { title: "Completed Disbursements", value: approvedApplications, icon: faCheckCircle, color: "from-green-400 to-green-600" }
  ];
    ...
    <div className="p-6 w-full min-h-screen max-w-7xl mx-auto">
      ...
      >
        ...
        <p className="text-gray-600">
          Review and manage ongoing car disbursements by users.
        </p>
      </motion.div>
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      <div className="flex justify-center space-x-2">
                        <button
                          onClick={() => handleView(loan.id, loan.car_id)}
